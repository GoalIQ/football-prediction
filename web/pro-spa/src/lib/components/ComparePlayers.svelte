<script lang="ts">
	...
	/* 6.8 (Rowanin palaute): vertailun jakokortti. 16.9: kortin rakennus
	 * siirtyi jaettuun `$lib/compareCard`-moduuliin, koska xP-lista sai oman
	 * "vertaa valitut" -napin ja kaksi rakentajaa samalle kortille eriytyisi
	 * seuraavassa muutoksessa. */
	let sharing = $state(false);
	async function shareImage() {
		if (sharing || !data) return;
		sharing = true;
		try {
			const method = await shareCompare(data);
