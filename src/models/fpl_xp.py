...
def set_p_start(mm: dict, p_start: float, status: str = "a", chance=None) -> dict:
    ...
    `status`/`chance` (12.9-korjaus, julkaisutarkistajan löydös): saatavuusportti
    ajetaan ohituksen JÄLKEEN. Ilman tätä ohitus epävarmalle pelaajalle (status
    != "a") tuottaisi xP:n josta pelaamistodennäköisyys puuttuu kokonaan, vaikka
    `/fpl` ja `/fpl/team-news` väittävät jaetusta moduulista (src/doubt_copy.py)
    että jokaisen epävarman rivin projektiopiste sisältää sen. Tämä ei riko
    "ohitus on viimeinen sana minuuttipassia vastaan" -periaatetta (14.8): se
    periaate koskee historiapohjaista minuuttimallia ja syvyysnormalisointia,
    ei kuluvan kierroksen loukkaantumis-/pelikuntolippua, jota ohitus ei ole
    koskaan tarkoittanut korvata.
    """
    out = dict(mm)
