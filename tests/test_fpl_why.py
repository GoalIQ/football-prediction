...
def test_template_drivers_come_from_facts(facts):
    """B.Fernandes: 84 min (>=60), xGI 0.68 (>=kynnys), pilkut -> kolme
    ensimmaista prioriteettijarjestyksessa. Ilman tata polkua drivers oli
    tyhja jokaisella rivilla eika chip-rivi renderoitynyt koskaan (19.8)."""
    ds = why.template_drivers(facts)
    assert ds == ["minutes", "attacking_output", "set_pieces"]
    assert all(d in why.DRIVERS for d in ds)


def test_template_drivers_survive_a_thin_player():
    """Negatiivinen kontrolli: tyhja faktalohko ei saa tuottaa yhtaan ajuria
    — keksitty chip olisi sama vikaluokka kuin keksitty luku."""
    assert why.template_drivers({}) == []


def test_template_drivers_defender_gets_clean_sheets():
    facts = {"position": "DEF", "expected_minutes": 90, "owned_pct": 3.2}
    ds = why.template_drivers(facts)
    assert ds == ["minutes", "clean_sheets", "differential"]


def test_template_drivers_cap_is_three():
    """Kortissa ei ole rivitysta: neljas chip putoaisi reunan yli, joten
    yli kolmen ajurin rivi ei saa syntya edes taydella faktalohkolla."""
    facts = {
        "position": "DEF",
        "expected_minutes": 90,
        "owned_pct": 3.2,
        "set_piece_duties": ["corners"],
        "last_season": {"xgi_per90": 0.4},
    }
    assert len(why.template_drivers(facts)) == 3
