...
posthog.init('phc_ASmq5P9R5goGTDxze3GkXHJqU6RsvMCNqunSVBMgGkn7',{api_host:'https://us.i.posthog.com',persistence:'memory',autocapture:false,person_profiles:'identified_only',capture_pageview:true,disable_surveys:true});
...
def fdr_rows_from_teams(teams: list[dict], gws: list[int]) -> list[dict]:
    ...
    10.9 KORJATTU (QUEUE: FDR-GRID-DGW): `by_gw` oli dict jonka avain oli
    gameweek, joten double gameweekissa jalkimmainen ottelu YLIKIRJOITTI
    edellisen ja katosi ruudukosta - samalla kun `next_n` laski sen mukaan
    ja `next_avg_cs_pct` sisalsi sen keskiarvossa. `cells` on nyt LISTA, ei
    per-GW dict (sama korjausmalli kuin `fpl_gw_xp.opponent_text`in
    "Lista, ei dict" -kommentti): jokainen nakyvan ikkunan sisalla oleva
    ottelu sailyy, ja `fdr_grid_html` ryhmittelee ne saraketta kohti oman
    `gw`-kenttansa perusteella renderointihetkella.
    """
    visible = set(gws)
    rows = []
    for t in teams:
        rows.append(
            {
                "team": t["name"],
                "cells": [f for f in t["fixtures"] if f["gw"] in visible],
    ...
    # 30.8 (portti k2): fail-closed. Jos rivilla on enemman otteluita kuin
    # nakyvan ikkunan `cells`, jokin MUU kuin double gameweek on pudottanut
    # ottelun (esim. artefaktin `next_n` ja `fixtures`-lista ovat eronneet
    # toisistaan) samalla kun Games-sarake ja Avg CS% laskevat sen mukaan.
    # Double gameweek ei enaa laukaise tata: molemmat ottelut ovat samassa
    # `cells`-listassa ja `fdr_grid_html` renderoi ne saman sarakkeen sisaan.
    ...
    for r in rows:
        ...
        if r["n"] > nakyvia:
            raise SystemExit(
                ...
                f"{nakyvia} nakyvassa ikkunassa. Games- ja Avg CS% -sarakkeet "
                f"laskisivat ottelun jota ruudukko ei nayta. Tarkista "
                f"artefaktin next_n ja fixtures ennen kuin sivu regeneroidaan.")
    return rows
...
def fdr_grid_html(c: dict) -> str:
    ...
    for r in c["fdr_rows"]:
        ...
        for i, g in enumerate(c["gws"]):
            ...
            # 10.9 (FDR-GRID-DGW): `r["cells"]` on lista kaikista nakyvan
            # ikkunan otteluista, ei yksi arvo per sarake, joten sarake
            # ryhmitellaan tassa oman `gw`-kentan perusteella. Double
            # gameweekissa fxs sisaltaa MOLEMMAT ottelut samassa solussa.
            fxs = [fx for fx in r["cells"] if fx and fx.get("gw") == g]
            if not fxs:
                cells.append(f'<td class="num{m}">-</td>')
            elif len(fxs) == 1:
                fx = fxs[0]
                ...
                # Double gameweek: >=2 ottelua samassa sarakkeessa. Yksi
                # td-tason vari valehtelisi toisesta ottelusta, joten vari
                # on linkkikohtainen.
                links = []
                for fx in fxs:
                    shown = format(float(fx["cs_pct"]), ".0f")
                ...
                cells.append(f'<td class="num{m}">{"".join(links)}</td>')
        rows.append(
            "<tr>"
...
<nav class="tooldir" aria-label="Free FPL tools">
  ...
  <a href="/fpl/predicted-lineups"><img src="/assets/cards/tools/predicted-lineups.webp" width="450" height="236" loading="lazy" decoding="async" alt=""><b>Predicted XI</b><span>Who starts at each club</span></a>
  <a href="/fpl/price-changes"><img src="/assets/cards/tools/price-changes.webp" width="450" height="236" loading="lazy" decoding="async" alt=""><b>Price changes</b><span>Who rises tonight</span></a>
...
<ul class="toollist">
  ...
  <li><a href="/fpl/model-xi">Model XI</a>: the best 15 our numbers fit in 100.0m. Not the squad the model plays (entry 116920).</li>
...
def _fmt_logged(row: dict) -> str:
    ...
    # 10.9 (QUEUE: LOGGED-SARAKE-KATKAISEE): `secs // 60` katkaisi sekunnit
    # eika pyoristanyt, joten 16 min 53 s luki "16 min" - katkaisu suosii
    # AINA meita, koska se nayttaa kirjauksen tuoreempana kuin se on (juuri
    # se suunta jonka lukija saisi kiinni laskemalla `gw_calls.json`:sta
    # itse). `round()` pyoristaa molempiin suuntiin.
