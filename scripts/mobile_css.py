...
/* A CAVEAT IS NOT A STATISTIC (26 Aug). A trimmed column's number may live
   behind "Show all columns"; a reason not to trust the number may not. This
   subline renders at every width, so the promoted-side caveat is on the page
   for a desktop reader too -- it used to be .m-only, which left the desktop
   reader with the word in a tooltip inside a column headed "Squad turnover". */
td .m-sub.is-caveat{display:block;font-size:11px;font-weight:400;opacity:.72;
  padding-top:2px;}
/* Projected goals under the opponent (10 Sep): a share card quotes these two
   numbers per fixture, so they must be readable at every width, phone
   included. Same treatment as the caveat subline: always block. */
td .m-sub.goals{display:block;font-size:11px;font-weight:400;opacity:.72;
  padding-top:2px;white-space:nowrap;}
/* Points drivers under the player name (10 Sep): the xP share card quotes
   this text, so it is readable at every width. */
td .m-sub.drv{display:block;font-size:11px;font-weight:400;opacity:.72;
  padding-top:2px;white-space:nowrap;}
